...
export class SymblEvents {
  subtitleHandlers: any = []; /** Handlers for the caption events **/
   ...
   * @param  type    handler type - can be `subtitle`, `insight`, and `transcript`
...
export class Insight {
   ...
   */
  get hints(): [{ key: string, value: number | boolean }] {
...
class SymblSocket {
  ...
  constructor(config: { confidenceThreshold: number, languageCode: string, insightsEnabled: boolean }, credentials: any) {
    this.id = ssCount++;
    this.config = config;
...
export class Symbl {
   ...
   * @return Promise that resolves as the meeting summary url.
   */
